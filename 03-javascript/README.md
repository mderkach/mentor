...
JavaScript здесь изучается не как набор конструкций, а как язык, который реально управляет поведением приложения.

Здесь важно понять:

- как работают данные, циклы и методы массивов;
- почему похожие конструкции делают не одно и то же;
- как устроены асинхронный код, Event Loop и очереди задач;
- где начинаются утечки памяти, лишняя нагрузка и проблемы в организации кода;
- какие встроенные и сложные API реально полезны инженеру.
...
- На middle и senior-уровне смотри не только на синтаксис, а на модель выполнения кода, планирование задач и границы языка.
...
- На senior-слое JavaScript уже читается как отдельная область инженерии: планирование задач, метапрограммирование, чувствительные к памяти API и требования к качеству кода.
- Держи в голове еще один фильтр: что здесь обычно понимают неправильно, где это ломается в проде и чем этот механизм отличается от соседних.
...
- [3.18 Прикладные паттерны выполнения кода: memoization, debounce, throttle, pub/sub и FSM](./40-senior/3.18-runtime-паттерны-memoization-debounce-throttle-pub-sub-и-fsm.md)
- [3.19 Правила качества JavaScript-кода и границы ответственности](./40-senior/3.19-javascript-quality-bar-code-conventions-и-архитектурные-границы.md)
- [3.20 Proxy, Reflect и границы метапрограммирования](./40-senior/3.20-proxy-reflect-и-границы-metaprogramming.md)
- [3.21 Продвинутое планирование задач: priorities, yielding и rendering budgets](./40-senior/3.21-advanced-scheduling-priorities-yielding-и-rendering-budgets.md)
- [3.22 WeakRef, FinalizationRegistry и паттерны, чувствительные к сборке мусора](./40-senior/3.22-weakref-finalizationregistry-и-gc-sensitive-patterns.md)
...
| Мидл | Понимать Event Loop, итераторы, встроенные структуры, память, профилирование и различия между похожими механизмами выполнения кода. |
| Сеньор / Senior+ | Проектировать прикладные паттерны выполнения кода, выбирать структуры данных, работать с метапрограммированием, планированием задач и чувствительными к памяти API, а также формировать JavaScript-стандарты команды. |
